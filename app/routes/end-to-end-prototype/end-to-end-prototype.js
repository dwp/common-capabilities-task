...
router.post('/end-to-end-prototype/v1/service-manager/manage-claims/add-or-manage-agents', (req, res) => {
	if (req.session.data.add.agents.or.manage == 'Add agents'){
		res.redirect("add-agents")
	} else {
		res.redirect("view-or-remove-agents")
	}
})
...
router.post('/end-to-end-prototype/v1/service-manager/manage-claims/remove-agent-from-org-leon', (req, res) => {
	if (req.session.data.removingAgentFromOrg.agentRemoveLeonAtkinson == 'yes'){
		res.redirect("manage-agents")
	} else {
		res.redirect("my-profile-agent-leon")
	}
})
